<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($_POST['action'] === "createnotice") {
            $noticeName = $_POST['noticeName'] ?? '';
            $noticeDesc = $_POST['noticeDesc'] ?? '';
            $noticeLink = $_POST['noticeLink'] ?? '';
            $noticeDate = $_POST['noticeDate'] ?? '';
            $noticeAddBy = $_POST['noticeAddBy'] ?? '';
            

            $newststmt = $pdo->prepare("INSERT INTO notice(notice_title, notice_desc, notice_link, notice_date, is_accepted, addby)
            VALUES (?, ?, ?, ?, ?, ?)");

            if ($newststmt->execute([$noticeName, $noticeDesc, $noticeLink, $noticeDate, 0, $noticeAddBy])) {
    ...
    elseif ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        if ($_GET['action'] === "getallNotice") {
            $getstmt = $pdo->prepare("SELECT * FROM notice");
            $getstmt->execute();
